...
volatile uint8        g_comm_fault = 0;

// =========================================================
// Private
// =========================================================
static uint8 s_timeout_counter    = 0;
static uint8 s_transfer_pending   = 0;   // 1 after a transfer is launched,
...
void Ipc_Init(void) {
    ...
    g_tx_packet.current_floor = 1U;
    g_tx_packet.target_floor  = 0U;
    g_tx_packet.lift_state    = 0U;   // IDLE
    g_tx_packet.motor_speed   = 0U;
    g_tx_packet.emergency     = 0U;
    g_tx_packet.cabin_buttons = 0U;
    g_tx_packet.checksum      = 0U;

    // RX packet — fully zeroed (prevents stale stack garbage)
    g_rx_packet.header        = 0x00U;
    g_rx_packet.current_floor = 0U;
    g_rx_packet.target_floor  = 0U;
    g_rx_packet.lift_state    = 0U;
    g_rx_packet.motor_speed   = 0U;
    g_rx_packet.emergency     = 0U;
    g_rx_packet.cabin_buttons = 0U;
    ...
    if (s_transfer_pending) {
        if (Spi1_GetState() == SPI_STATUS_IDLE) {
            if (Ipc_ValidatePacket(&g_rx_packet)) {
                ...
                ENTER_CRITICAL();
                g_rx_packet.target_floor  = 0U;   // ignore corrupted target
                g_rx_packet.lift_state    = 0U;   // assume IDLE (safe)
                g_rx_packet.emergency     = 0U;   // don't inject false emergencies
                EXIT_CRITICAL();
